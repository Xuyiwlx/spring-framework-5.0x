...
public class AnnotatedBeanDefinitionReader {
	 ...
	 * 看到其父类实现了BeanDefinitionRegistry,那么BeanDefinitionRegistry的作用是什么呢
	 * BeanDefinitionRegistry,顾名思议是BeanDefinition的注册器
	...
	<T> void doRegisterBean(Class<T> annotatedClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		* 这里需要注意的是,qualifiers这个变量是Annotation类型的数组,
		* 里面存的不仅仅是@Qualifier,理论上存的是一切注解,所以可以看到
		* 下面的代码,spring去循环了这个数组,然后依次判断注解是否包含@Primary/@Lazy
		* */
		if (qualifiers != null) {
			for (Class<? extends Annotation> qualifier : qualifiers) {
				if (Primary.class == qualifier) {
					...
					/*
					* 如果使用了除@Primary和@Lazy以外的其他注解,则为该bean添加一个根据名字自动
					* 装配的限定符
					* */
		...
		//BeanDefinitionHolder也是一个数据结构(可以理解为Map)
		BeanDefinitionHolder definitionHolder = new BeanDefinitionHolder(abd, beanName);
		//ScopedProxyMode 这个知识点比较复杂,需要结合web去理解(springmvc)
		definitionHolder = AnnotationConfigUtils.applyScopedProxyMode(scopeMetadata, definitionHolder, this.registry);
		/*
		* 把上述的holder注册给registry,
		* registry就是AnnotationConfigApplicationContext,
		* AnnotationConfigApplicationContext在初始化的时候通过调用父类的构造方法,
		* 实例化了一个DefaultListableBeanFactory,registerBeanDefinition就是把holder
		* 包含的信息注册到DefaultListableBeanFactory
		* */
